<?php
define('EP_INSTAPRESENT', 8388608); // 8388608 = 2^23
/**
 ** WP Present Core
 **
 ** @since 0.9.4
 **/
class insta_present_Core {
	...
	/* Metakeys */
	const METAKEY_PREFIX    = 'insta_present_';
	...
	const TEXT_DOMAIN        = 'insta-present';
	...
	/**
	 * Adds a insta-present body class
	 *
	 * @filter body_class
	 * @return array
	 */
	function filter_body_class( $classes ) {
		$classes[] = 'insta-present';
		return $classes;
	}
...
//		add_editor_style( plugins_url( '/insta-present/css/reset.css' ) );
		add_editor_style( plugins_url( '/insta-present/js/reveal.js/css/reveal.css' ) );
		add_editor_style( plugins_url( '/insta-present/js/reveal.js/css/theme/' . self::DEFAULT_THEME ) );
		add_editor_style( plugins_url( '/insta-present/js/reveal.js/lib/css/zenburn.css' ) );
		add_editor_style( plugins_url( '/insta-present/css/custom.css?v=' . self::REVISION ) );

		//TODO: Make this work to support backgrounds
		//add_editor_style( plugins_url( '/insta-present/css/tinymce.css.php?v=' . self::REVISION . '&post=' . $_REQUEST['post'] ) );
	}
	...
	function action_init_add_endpoints() {
		add_rewrite_endpoint( 'embed', EP_INSTAPRESENT );
		add_rewrite_endpoint( 'fullscreen', EP_INSTAPRESENT );
	}
	...
	public function taxonomy_edit_form( $term, $taxonomy ) {
		...
		$tax_query = new WP_Query( array(
			'post_type' => insta_present_Core::POST_TYPE_TAXONOMY,
		...
		<div id="dialog" class="media-modal" title="Edit <?php echo self::POST_TYPE_SINGULAR; ?>" style="display: none;">
			<div class="modal-inner-left">
				<?php insta_present_Modal_Customizer::instance()->render(); ?>
			</div>
